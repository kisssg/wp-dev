...
if (!class_exists('LittleDino_breadcrumbs')) {
    /**
     * Breadcrumbs area
     *
     *
     * @package littledino\templates
     * @author WebGeniusLab <webgeniuslab@gmail.com>
     * @since 1.0.0
     * @version 1.1.5
     */
    class LittleDino_breadcrumbs
    {
        ...
        {
			$this->text_home     = esc_html__('Home', 'littledino'); // text for the 'Home' link
			$this->text_category = esc_html__('Archive by Category "%s"', 'littledino'); // text for a category page
			$this->text_search   = esc_html__('Search Results for "%s" Query', 'littledino'); // text for a search results page
			$this->text_tag      = esc_html__('Posts Tagged "%s"', 'littledino'); // text for a tag page
			$this->text_author   = esc_html__('Articles Posted by %s', 'littledino'); // text for an author page
			$this->text_404      = esc_html__('Error 404', 'littledino'); // text for the 404 page
			$this->text_page     = esc_html__('Page %s', 'littledino'); // text 'Page N'
			$this->text_cpage    = esc_html__('Comment Page %s', 'littledino'); // text 'Comment Page N'
        }
        ...
        public function __construct ()
        {
			...
			if (is_home() || is_front_page()) {
                ...
                // Portfolio Taxonomies Archive
                    ...
                    $archive_link = get_post_type_archive_link(LittleDino::get_option('portfolio_slug') ?: 'portfolio');
                    $is_category = is_tax('portfolio-category') ? true : false;
                    $tax_name = get_queried_object()->name;

                    if (get_query_var('paged')) {
                        $out .= $sep
                            . sprintf($link, $archive_link, esc_html__('Portfolio', 'littledino'))
                            . $sep
                            . $before
				...
				// Custom Post Type
                    ...
                    && is_product()
                    && function_exists('littledino_woocommerce_breadcrumb')
                ) {
                    $args = [];
                    $args['delimiter'] = $sep;
                    if ($show_home_link) $out .= $sep;
                    $out .= littledino_woocommerce_breadcrumb($args);
	   			}
				$out .= $wrap_after;
			}

			echo LittleDino::render_html($out);
    	}
    }

    new LittleDino_breadcrumbs();
}
